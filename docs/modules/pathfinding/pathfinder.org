* LrgPathfinder
:PROPERTIES:
:CUSTOM_ID: lrgpathfinder
:END:
A final GObject implementing the A* pathfinding algorithm for finding optimal paths on a navigation grid.

** Type Information
:PROPERTIES:
:CUSTOM_ID: type-information
:END:
- *Type Name*: =LrgPathfinder=
- *Type ID*: =LRG_TYPE_PATHFINDER=
- *Category*: Final GObject (cannot be subclassed)
- *Header*: =lrg-pathfinder.h=

** Description
:PROPERTIES:
:CUSTOM_ID: description
:END:
=LrgPathfinder= implements the A* algorithm for finding optimal paths from a start position to a goal position on a navigation grid. Key features:

- *A-star Search*: Optimal pathfinding with customizable heuristics
- *Multiple Heuristics*: Built-in Manhattan, Euclidean, Chebyshev, and Octile distance
- *Custom Heuristics*: Support for application-specific distance calculations
- *Path Smoothing*: Optional post-processing to smooth paths
- *Reachability Checks*: Boolean convenience wrapper around path search
- *Performance Metrics*: Track nodes explored for optimization

** Creating a Pathfinder
:PROPERTIES:
:CUSTOM_ID: creating-a-pathfinder
:END:
*** lrg_pathfinder_new()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_new
:END:
#+begin_src C
LrgPathfinder *
lrg_pathfinder_new (LrgNavGrid *grid)
#+end_src

Creates a new pathfinder for the given grid.

*Parameters:*

- =grid=: The navigation grid (can be NULL, but must be set before pathfinding)

*Returns:* (transfer full) A new =LrgPathfinder=

*Example:*

#+begin_src C
g_autoptr(LrgNavGrid) grid = lrg_nav_grid_new(100, 100);
g_autoptr(LrgPathfinder) pf = lrg_pathfinder_new(grid);
#+end_src

** Grid Management
:PROPERTIES:
:CUSTOM_ID: grid-management
:END:
*** lrg_pathfinder_get_grid()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_get_grid
:END:
#+begin_src C
LrgNavGrid *
lrg_pathfinder_get_grid (LrgPathfinder *self)
#+end_src

Gets the navigation grid.

*Returns:* (transfer none) The navigation grid

*** lrg_pathfinder_set_grid()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_set_grid
:END:
#+begin_src C
void
lrg_pathfinder_set_grid (LrgPathfinder *self,
                         LrgNavGrid    *grid)
#+end_src

Sets or changes the navigation grid.

*Example:*

#+begin_src C
g_autoptr(LrgNavGrid) new_grid = lrg_nav_grid_new(200, 200);
lrg_pathfinder_set_grid(pathfinder, new_grid);
#+end_src

** Finding Paths
:PROPERTIES:
:CUSTOM_ID: finding-paths
:END:
*** lrg_pathfinder_find_path()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_find_path
:END:
#+begin_src C
LrgPath *
lrg_pathfinder_find_path (LrgPathfinder  *self,
                          gint            start_x,
                          gint            start_y,
                          gint            end_x,
                          gint            end_y,
                          GError        **error)
#+end_src

Finds a path from start to end using A*.

*Parameters:*

- =start_x=, =start_y=: Starting position
- =end_x=, =end_y=: Goal position
- =error=: (nullable) Return location for error

*Returns:* (transfer full) (nullable) The path, or NULL on error

*Error Codes:* (from =LRG_PATHFINDING_ERROR=)

- =LRG_PATHFINDING_ERROR_NO_GRID= - No grid set on pathfinder
- =LRG_PATHFINDING_ERROR_INVALID_START= - Start position invalid or blocked
- =LRG_PATHFINDING_ERROR_INVALID_GOAL= - Goal position invalid or blocked
- =LRG_PATHFINDING_ERROR_NO_PATH= - Search exhausted all candidates without reaching the goal
- =LRG_PATHFINDING_ERROR_ITERATION_LIMIT= - Budget exhausted with candidates remaining; reachability is unknown

*Example:*

#+begin_src C
GError *error = NULL;
LrgPath *path = lrg_pathfinder_find_path(pathfinder, 10, 10, 90, 90, &error);

if (path != NULL) {
    g_print("Found path with %u waypoints\n", lrg_path_get_length(path));
    lrg_path_free(path);
} else {
    g_warning("Pathfinding failed: %s", error->message);
    g_error_free(error);
}
#+end_src

** Reachability Checks
:PROPERTIES:
:CUSTOM_ID: reachability-checks
:END:
*** lrg_pathfinder_is_reachable()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_is_reachable
:END:
#+begin_src C
gboolean
lrg_pathfinder_is_reachable (LrgPathfinder *self,
                             gint           start_x,
                             gint           start_y,
                             gint           end_x,
                             gint           end_y)
#+end_src

Runs the same search as =find_path()= and frees the result. It honors the
iteration budget and returns =FALSE= for both search errors and unreachable goals.

*Returns:* =TRUE= if a path exists

*Use Case:* Callers that only need a boolean answer. When a path is needed,
call =find_path()= directly to avoid searching twice.

*Example:*

#+begin_src C
if (lrg_pathfinder_is_reachable(pathfinder, 10, 10, 90, 90)) {
    g_print("Goal is reachable\n");
}
#+end_src

** Path Smoothing
:PROPERTIES:
:CUSTOM_ID: path-smoothing
:END:
*** lrg_pathfinder_get_smoothing()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_get_smoothing
:END:
#+begin_src C
LrgPathSmoothingMode
lrg_pathfinder_get_smoothing (LrgPathfinder *self)
#+end_src

Gets the path smoothing mode.

*Returns:* The smoothing mode

*** lrg_pathfinder_set_smoothing()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_set_smoothing
:END:
#+begin_src C
void
lrg_pathfinder_set_smoothing (LrgPathfinder        *self,
                              LrgPathSmoothingMode  mode)
#+end_src

Sets the path smoothing mode.

*Smoothing Modes:* (from =lrg-enums.h=)

- =LRG_PATH_SMOOTHING_NONE= - No smoothing (default)
- =LRG_PATH_SMOOTHING_SIMPLE= - Removes collinear waypoints, retaining the search cost
- =LRG_PATH_SMOOTHING_BEZIER= - Reserved; currently leaves the path unchanged

*Example:*

#+begin_src C
/* Enable path smoothing for smoother movement */
lrg_pathfinder_set_smoothing(pathfinder, LRG_PATH_SMOOTHING_SIMPLE);
#+end_src

** Iteration Control
:PROPERTIES:
:CUSTOM_ID: iteration-control
:END:
*** lrg_pathfinder_get_max_iterations()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_get_max_iterations
:END:
#+begin_src C
guint
lrg_pathfinder_get_max_iterations (LrgPathfinder *self)
#+end_src

Gets the maximum iterations limit.

*Returns:* Maximum iterations (0 = unlimited)

*** lrg_pathfinder_set_max_iterations()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_set_max_iterations
:END:
#+begin_src C
void
lrg_pathfinder_set_max_iterations (LrgPathfinder *self,
                                   guint          max_iterations)
#+end_src

Sets the maximum number of node expansions per search. Zero means unlimited.
The start and goal each count when expanded (a start equal to the goal returns
immediately with zero expansions). Reopened nodes count again. Reaching the goal
on the final allowed expansion succeeds. An exhausted candidate set returns
=NO_PATH=; an unfinished search returns =ITERATION_LIMIT=. The budget does not
bound wall-clock time or the initial scan of cell costs.

#+begin_src C
g_autoptr(GError) error = NULL;
g_autoptr(LrgPath) path = NULL;
lrg_pathfinder_set_max_iterations (pathfinder, 1000);
path = lrg_pathfinder_find_path (pathfinder, 0, 0, 90, 90, &error);
if (g_error_matches (error, LRG_PATHFINDING_ERROR,
                     LRG_PATHFINDING_ERROR_ITERATION_LIMIT))
{
    /* Defer a fresh search with a larger budget; do not mark the goal blocked. */
}
#+end_src

*Use Case:* Prevent excessive computation on large grids with blocked goals

*Parameters:*

- =max_iterations=: Maximum (0 = unlimited)

*Example:*

#+begin_src C
/* Limit to 10000 iterations to prevent performance hiccups */
lrg_pathfinder_set_max_iterations(pathfinder, 10000);
#+end_src

** Heuristic Functions
:PROPERTIES:
:CUSTOM_ID: heuristic-functions
:END:
*** lrg_pathfinder_set_heuristic()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_set_heuristic
:END:
#+begin_src C
void
lrg_pathfinder_set_heuristic (LrgPathfinder    *self,
                              LrgHeuristicFunc  func,
                              gpointer          user_data,
                              GDestroyNotify    destroy)
#+end_src

Sets a custom heuristic function. If NULL, selects Manhattan distance for
cardinal movement or octile distance for diagonal movement, scaled by the
lowest current cell cost (capped at 1). Zero-cost cells and custom neighbor
graphs use Dijkstra's algorithm. The bound is recalculated for each search
because cell costs are mutable. Custom heuristics must return finite, nonnegative lower
bounds and zero at the goal to guarantee optimal paths. Consistency is optional:
a closed node is reopened when a cheaper route is found. Callbacks must not
mutate the grid or pathfinder during a search.

*Parameters:*

- =func=: (nullable) Custom heuristic function
- =user_data=: Arbitrary data for the function
- =destroy=: Cleanup function for user_data

*Heuristic Function Signature:*

#+begin_src C
typedef gfloat (*LrgHeuristicFunc) (gint     x1,
                                    gint     y1,
                                    gint     x2,
                                    gint     y2,
                                    gpointer user_data);
#+end_src

Returns estimated cost from (x1,y1) to (x2,y2). The built-in distance helpers
accept the full signed integer coordinate range without integer overflow.

*** Built-in Heuristics
:PROPERTIES:
:CUSTOM_ID: built-in-heuristics
:END:
**** lrg_heuristic_manhattan()
:PROPERTIES:
:CUSTOM_ID: lrg_heuristic_manhattan
:END:
#+begin_src C
gfloat
lrg_heuristic_manhattan (gint     x1,
                         gint     y1,
                         gint     x2,
                         gint     y2,
                         gpointer user_data)
#+end_src

Manhattan (taxicab) distance heuristic.

*Formula:* =|x2-x1| + |y2-y1|=

*Best for:* Cardinal-only movement (no diagonals)

**** lrg_heuristic_euclidean()
:PROPERTIES:
:CUSTOM_ID: lrg_heuristic_euclidean
:END:
#+begin_src C
gfloat
lrg_heuristic_euclidean (gint     x1,
                         gint     y1,
                         gint     x2,
                         gint     y2,
                         gpointer user_data)
#+end_src

Euclidean distance heuristic.

*Formula:* =sqrt((x2-x1)^2 + (y2-y1)^2)=

*Best for:* Continuous movement

**** lrg_heuristic_chebyshev()
:PROPERTIES:
:CUSTOM_ID: lrg_heuristic_chebyshev
:END:
#+begin_src C
gfloat
lrg_heuristic_chebyshev (gint     x1,
                         gint     y1,
                         gint     x2,
                         gint     y2,
                         gpointer user_data)
#+end_src

Chebyshev distance heuristic (diagonal cost = 1).

*Formula:* =max(|x2-x1|, |y2-y1|)=

*Best for:* Chess king movement (equal diagonal/cardinal cost)

**** lrg_heuristic_octile()
:PROPERTIES:
:CUSTOM_ID: lrg_heuristic_octile
:END:
#+begin_src C
gfloat
lrg_heuristic_octile (gint     x1,
                      gint     y1,
                      gint     x2,
                      gint     y2,
                      gpointer user_data)
#+end_src

Octile distance heuristic (diagonal cost = sqrt(2)).

*Formula:* Approximate diagonal cost

*Best for:* Most game movement (8-directional with realistic costs)

*Example:*

#+begin_src C
/* For unit-cost terrain, Euclidean distance is a valid lower bound. */
lrg_pathfinder_set_heuristic(pathfinder,
                             lrg_heuristic_euclidean,
                             NULL,
                             NULL);
#+end_src

** Performance Metrics
:PROPERTIES:
:CUSTOM_ID: performance-metrics
:END:
*** lrg_pathfinder_get_last_nodes_explored()
:PROPERTIES:
:CUSTOM_ID: lrg_pathfinder_get_last_nodes_explored
:END:
#+begin_src C
guint
lrg_pathfinder_get_last_nodes_explored (LrgPathfinder *self)
#+end_src

Gets the number of nodes explored in the last pathfinding operation.

*Returns:* Nodes explored count

*Use Case:* Performance profiling and optimization

*Example:*

#+begin_src C
LrgPath *path = lrg_pathfinder_find_path(pathfinder, 10, 10, 90, 90, NULL);
if (path != NULL) {
    guint nodes = lrg_pathfinder_get_last_nodes_explored(pathfinder);
    g_print("Explored %u nodes\n", nodes);
}
#+end_src

** Complete Example
:PROPERTIES:
:CUSTOM_ID: complete-example
:END:
#+begin_src C
#include <glib.h>
#include <libregnum.h>

int main(void) {
    g_autoptr(GError) error = NULL;

    /* Create grid with obstacles */
    g_autoptr(LrgNavGrid) grid = lrg_nav_grid_new(50, 50);
    lrg_nav_grid_set_allow_diagonal(grid, TRUE);

    /* Create wall obstacle */
    for (gint i = 0; i < 40; i++) {
        lrg_nav_grid_set_blocked(grid, 25, i, TRUE);
    }

    /* Create pathfinder */
    g_autoptr(LrgPathfinder) pf = lrg_pathfinder_new(grid);
    lrg_pathfinder_set_smoothing(pf, LRG_PATH_SMOOTHING_SIMPLE);
    lrg_pathfinder_set_max_iterations(pf, 5000);

    /* Find path */
    g_autoptr(LrgPath) path = lrg_pathfinder_find_path(pf, 10, 25, 40, 25, &error);
    if (path == NULL) {
        g_warning("Pathfinding failed: %s", error->message);
        return 1;
    }

    /* Analyze result */
    guint length = lrg_path_get_length(path);
    guint nodes = lrg_pathfinder_get_last_nodes_explored(pf);
    gfloat cost = lrg_path_get_total_cost(path);

    g_print("Path found: %u waypoints, %u nodes explored, cost: %.2f\n",
            length, nodes, cost);

    /* Print waypoints */
    for (guint i = 0; i < length; i++) {
        gint x, y;
        if (lrg_path_get_point(path, i, &x, &y)) {
            g_print("  [%u] (%d, %d)\n", i, x, y);
        }
    }

    return 0;
}
#+end_src

** Algorithm Details
:PROPERTIES:
:CUSTOM_ID: algorithm-details
:END:
The A* algorithm works by:

1. *Open Set*: An indexed binary min-heap maintains candidates; insertion,
   removal, and priority updates take logarithmic time in the candidate count.
   Equal priorities use discovery order for deterministic tie-breaking.
2. *Closed Set*: Tracks explored nodes, reopening them when their cost improves
3. *Cost Calculation*: =f(node) = g(node) + h(node)= where:
   - =g(node)= = actual cost from start to node
   - =h(node)= = heuristic estimate from node to goal
4. *Expansion*: Always expands the node with lowest f-cost
5. *Termination*: Stops when goal is reached or open set is exhausted

** Related Types
:PROPERTIES:
:CUSTOM_ID: related-types
:END:
- [[file:nav-grid.org][LrgNavGrid]] - Navigation grid
- [[file:path.org][LrgPath]] - Path result
- [[file:nav-cell.org][LrgNavCell]] - Individual cells
