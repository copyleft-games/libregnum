...
:PROPERTIES:
:CUSTOM_ID: the-lrgevent-interface
:END:
...
:PROPERTIES:
:CUSTOM_ID: implementing-lrgevent
:END:
#+begin_src C
#include <libregnum.h>
...
:PROPERTIES:
:CUSTOM_ID: the-lrgeventlistener-interface
:END:
...
:PROPERTIES:
:CUSTOM_ID: implementing-lrgeventlistener
:END:
#+begin_src C
#include <libregnum.h>
...
:PROPERTIES:
:CUSTOM_ID: registering-listeners
:END:
#+begin_src C
/* Create listener */
...
*** Dispatch and Lifetime Rules
:PROPERTIES:
:CUSTOM_ID: dispatch-lifetime
:END:
Each emission captures the current registrations and their priorities before
notifying listeners. Higher priorities run first; ties follow registration
order. A priority changed by a callback affects later emissions, including a
nested emission, while the outer emission retains its captured order.

Each call to =register()= creates a separate registration. Registering an object
twice delivers twice. =unregister()= removes its oldest remaining registration;
=unregister_by_id()= removes all matches. Removed registrations are skipped if
their turn has not arrived. Re-registering the same object creates a new
registration that cannot revive a removed entry in an active emission.
=clear()= invalidates all pending registrations.

Nested emissions capture their own registrations. For example, a listener
registered by an outer callback can receive a nested event immediately, but
will not receive that outer event. Use a distinct event object for a distinct
nested event; cancellation belongs to the event object.

The bus retains itself, the event, and captured listeners until dispatch and
completion signals finish. A callback may release its owner's references;
objects are destroyed only after the last remaining reference is released.
The optional context pointer remains caller-owned and must stay valid through
dispatch. This is synchronous dispatch, not a thread-safe queue: use one thread
or external serialization, and keep listener accessors free of bus mutations.

Capturing and sorting priorities costs O(N log N) per emission. Checking whether
a registration was removed is O(1), including after callback mutations.

To preserve an original event, copy it using the concrete event type's copy
function and emit that copy. There is no generic =lrg_event_bus_emit_copy()= API.
...
:PROPERTIES:
:CUSTOM_ID: deckbuilder-integration
:END:
...
:PROPERTIES:
:CUSTOM_ID: lrgeventlistener-interface
:END:
| Function                                                | Description                   |
|---------------------------------------------------------+-------------------------------|
...
:CUSTOM_ID: related
:END:
- [[file:index.org][Core Module Overview]] - Core module documentation
- [[file:../deckbuilder/index.org][Deckbuilder Module]] - Card game implementation using events
