...
Call =lrg_input_manager_poll(lrg_input_manager_get_default())= once at the
start of each input frame, before querying bindings. Engine and game-template
updates do this automatically. A custom loop owns this call; do not poll again
inside callbacks or fixed simulation steps of the same frame.

Axis state queries compare the previous and current snapshots. Crossing the
configured threshold produces one pressed frame, staying beyond it remains
down, and falling below it produces one released frame. Every query within a
frame sees the same state, including copies of a binding used by several actions.
Skipping queries does not defer an edge to a later frame.

Before the first poll, axis state is neutral. A held axis on the first poll
produces a press. Zero stays neutral even with threshold zero; other exact
threshold values activate in the configured direction. Disconnecting a held
controller, disabling its source, or removing it produces one release frame on
the next poll. Disabling the entire manager suppresses all queries and clears
history; the next poll after re-enabling treats held axes as new presses.
...
/* Get the live axis value (independent of frame edge snapshots) */
...
- Axis pressed/down/released state comes from the input manager frame snapshots
- =get_axis_value()= remains a live query of enabled input sources
