...
:PROPERTIES:
:CUSTOM_ID: multiple-binding-types
:END:
...
Axis state queries use the snapshots captured by =lrg_input_manager_poll()=.
Engine and game-template updates poll automatically before their input consumers.
Custom loops must poll the default manager once per input frame. Do not combine
engine, template, and explicit polling as competing frame owners. Press and
release results are stable for the entire frame, even when several actions share
a binding. The live analog-value APIs remain available for direct source queries.

Software input injection is immediately queryable and survives the next manager
poll. Injected keyboard, mouse, and gamepad button edges remain visible for that
polled frame. A software key tap stays down for one polled frame, then releases
on the next. Applications using the manager should not separately call
=lrg_input_software_update()= in the same frame.

*** YAML Serialization
:PROPERTIES:
:CUSTOM_ID: yaml-serialization
:END:
...
:PROPERTIES:
:CUSTOM_ID: creating-an-action-map
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: dynamic-binding-creation
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: detecting-binding-changes
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: integration-with-game-loop
:END:
...
:END:
- See [[file:input-action.org][Input Actions]] for action management
- See [[file:input-map.org][Input Map]] for map/serialization features
- See [[file:input-binding.org][Input Bindings]] for binding details
- See [[../examples/input-handling.md][Input Handling Example]] for complete code sample
