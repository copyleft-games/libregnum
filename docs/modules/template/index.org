...
#+begin_quote
*[[../../index.md][Home]]* > *[[../index.md][Modules]]* > Template
...
*** Game Templates
:PROPERTIES:
:CUSTOM_ID: game-templates
:END:
Base templates for different game types:

| Template                                                  | Description                                        |
|-----------------------------------------------------------+----------------------------------------------------|
| [[file:templates/game-template.org][LrgGameTemplate]]      | Base template with lifecycle, input, and game feel |
| [[file:templates/game-2d-template.org][LrgGame2DTemplate]] | 2D games with virtual resolution and cameras       |
| [[file:templates/game-3d-template.org][LrgGame3DTemplate]] | 3D games with first/third-person cameras           |

*** Genre Templates
:PROPERTIES:
:CUSTOM_ID: genre-templates
:END:
Pre-built templates for specific game genres:

| Template                                                                         | Description                             |
|----------------------------------------------------------------------------------+-----------------------------------------|
| [[file:templates/idle-template.org][LrgIdleTemplate]]                             | Idle/incremental games with big numbers |
| [[file:templates/deckbuilder-template.org][LrgDeckbuilderTemplate]]               | Card games (combat and poker variants)  |
| [[file:templates/platformer-template.org][LrgPlatformerTemplate]]                 | 2D platformers with physics             |
| [[file:templates/top-down-template.org][LrgTopDownTemplate]]                      | Top-down games (RPG, action)            |
| [[file:templates/shooter-template.org][LrgTwinStickTemplate / LrgShmupTemplate]]  | 2D shooter variants                     |
| [[file:templates/fps-template.org][LrgFPSTemplate]]                               | First-person shooters                   |
| [[file:templates/third-person-template.org][LrgThirdPersonTemplate]]              | Third-person action games               |
| [[file:templates/racing-template.org][LrgRacing2DTemplate / LrgRacing3DTemplate]] | Racing games                            |
| [[file:templates/tycoon-template.org][LrgTycoonTemplate]]                         | Tycoon/management games                 |
...
| System                                         | Description                                              |
|------------------------------------------------+----------------------------------------------------------|
| [[file:systems/menu-states.org][Menu States]]   | Main menu, pause, settings, loading, error, confirmation |
| [[file:systems/input-buffer.org][Input Buffer]] | Frame-perfect input buffering for action games           |
| [[file:systems/object-pool.org][Object Pool]]   | Object pooling for performance                           |
| [[file:systems/game-feel.org][Game Feel]]       | Hit stop, screen shake, time scale                       |
| [[file:systems/engagement.org][Engagement]]     | Statistics, daily rewards, difficulty                    |
| [[file:systems/mixins.org][Mixins]]             | LrgIdleMixin, LrgDeckMixin interfaces                    |
...
| Example                                         | Description                       |
|-------------------------------------------------+-----------------------------------|
| [[file:examples/minimal-game.org][Minimal Game]] | Simplest possible template usage  |
| [[file:examples/idle-game.org][Idle Game]]       | Cookie clicker-style game         |
| [[file:examples/deckbuilder.org][Deckbuilder]]   | Roguelike card combat             |
| [[file:examples/platformer.org][Platformer]]     | 2D platformer with wall mechanics |
...
| Guide                                                       | Description                                      |
|-------------------------------------------------------------+--------------------------------------------------|
| [[../../guides/template-best-practices.md][Best Practices]] | Coordinate spaces, async resize, common pitfalls |
...
:PROPERTIES:
:CUSTOM_ID: pause-menu-with-audio-ducking
:END:
#+begin_src C
/* Create pause menu */
...
:PROPERTIES:
:CUSTOM_ID: example-custom-main-menu
:END:
#+begin_src C
static void
...
:PROPERTIES:
:CUSTOM_ID: example-custom-pause-behavior
:END:
#+begin_src C
static void
...
** Loading State
:PROPERTIES:
:CUSTOM_ID: loading-state
:END:
=LrgTemplateLoadingState= executes one synchronous task per update. Successful
tasks advance its progress bar; failures stop the queue and emit =failed=.
After every task succeeds and the minimum display time elapses, =complete=
fires once. A single expensive task can still delay a frame.

See [[file:states/loading.org][Loading state asset integration]] for manager injection, supported formats,
reload behavior, and retry semantics.
...
| Property               | Type    | Default      | Description                         |
|------------------------+---------+--------------+-------------------------------------|
| =progress=             | double  | 0.0          | Read-only successful-task fraction   |
| =status-text=          | string  | "Loading..." | Main loading text                   |
| =show-progress-bar=    | boolean | TRUE         | Show progress bar                   |
| =show-percentage=      | boolean | TRUE         | Show numeric progress               |
| =minimum-display-time= | double  | 0.5          | Minimum seconds before completion   |

*** Example: Asset Loading
:PROPERTIES:
:CUSTOM_ID: example-asset-loading
:END:
#+begin_src C
LrgTemplateLoadingState *loading = lrg_template_loading_state_new ();
lrg_template_loading_state_set_status_text (loading, "Loading Level...");
lrg_template_loading_state_set_asset_manager (loading, assets);
lrg_template_loading_state_add_asset (loading, "levels/intro.yaml");
lrg_template_loading_state_add_asset (loading, "textures/player.png");

/* Connect complete/failed handlers before pushing. The manager owns loading. */
lrg_game_state_manager_push (manager, LRG_GAME_STATE (loading));
#+end_src

** Error State
:PROPERTIES:
:CUSTOM_ID: error-state
:END:
=LrgTemplateErrorState= provides error recovery UI.

*** Features
:PROPERTIES:
:CUSTOM_ID: features-4
:END:
...
:PROPERTIES:
:CUSTOM_ID: example-error-handling
:END:
#+begin_src C
static void
...
:PROPERTIES:
:CUSTOM_ID: example-delete-confirmation
:END:
#+begin_src C
LrgTemplateConfirmationState *confirm =
    lrg_template_confirmation_state_new_with_message (
...
:PROPERTIES:
:CUSTOM_ID: example-using-statistics
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: implementing-daily-rewards
:END:
#+begin_src C
struct _MyGameState
{
...
:PROPERTIES:
:CUSTOM_ID: implementing-dynamic-difficulty
:END:
#+begin_src C
struct _MyGameState
{
...
:PROPERTIES:
:CUSTOM_ID: using-dynamic-difficulty
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: genre-specific-templates-phase-3
:END:
...
:PROPERTIES:
:CUSTOM_ID: genre-template-summary
:END:
...
:PROPERTIES:
:CUSTOM_ID: example-implementing-lrgidlemixin
:END:
#+begin_src C
static gdouble
...
:PROPERTIES:
:CUSTOM_ID: example-basic-idle-game
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: example-basic-deckbuilder
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: example-slay-the-spire-combat
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: default-hand-scoring
:END:
...
:PROPERTIES:
:CUSTOM_ID: example-balatro-style-poker
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: object-pooling-phase-35
:END:
...
:PROPERTIES:
:CUSTOM_ID: implementing-lrgpoolable
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: example-bullet-pool-for-shoot-em-up
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: particle-system-example
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: genre-templates-phase-5
:END:
...
:PROPERTIES:
:CUSTOM_ID: lrgshooter2dtemplate
:END:
...
:PROPERTIES:
:CUSTOM_ID: lrgtwinsticktemplate
:END:
...
:PROPERTIES:
:CUSTOM_ID: creating-custom-genre-templates
:END:
...
:CUSTOM_ID: see-also
:END:
- *[[../gamestate/index.md][Game State Module]]* - State management system
- *[[../ui/index.md][UI Module]]* - Widget system
- *[[../settings/index.md][Settings Module]]* - Settings persistence
- *[[../audio/index.md][Audio Module]]* - Audio ducking
- *[[../save/index.md][Save Module]]* - Save/load persistence
- *[[../deckbuilder/index.md][Deckbuilder Module]]* - Card, deck, and combat systems
- *[[../idle/index.md][Idle Module]]* - BigNumber, prestige, and automation
- *[[../economy/index.md][Economy Module]]* - Resource production and consumption
