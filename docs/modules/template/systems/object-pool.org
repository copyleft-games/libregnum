...
typedef enum {
    LRG_POOL_GROWTH_FIXED,     /* Don't grow, return NULL */
...
:CUSTOM_ID: iteration
:END:
Iteration captures the active acquisitions at entry, in reverse storage order.
Callbacks may release the current object or another object, clear the pool,
release all active objects, acquire objects, or start a nested iteration.
Released acquisitions are skipped. A new acquisition waits for a subsequent
iteration, even when it reuses a captured object's address. Nested iterations
capture their own current set. Returning =FALSE= stops iteration early.

The pool and captured objects stay alive until iteration ends. This prevents
callbacks from receiving freed objects after =clear()= or owner destruction.
Object state is still mutable: a reference snapshot is not a copy of each
object's properties. Storage order can change after releases and is not a
stable acquisition order. Iteration uses O(N) temporary memory. Each acquisition
allocates a small bookkeeping record; prewarming avoids constructing gameplay
objects, but does not make acquisition or iteration allocation-free.

Changing =LrgPoolable='s active flag alone does not return an object to the
pool; call =lrg_object_pool_release()= to update pool membership.
...
{
    BulletUpdate *update = user_data;
    MyBullet *bullet = MY_BULLET (poolable);

    bullet->x += bullet->velocity_x * update->delta;
    bullet->y += bullet->velocity_y * update->delta;
    if (is_out_of_bounds (bullet))
        lrg_object_pool_release (update->pool, poolable);
    return TRUE;
}

/* In the game update function: */
BulletUpdate update = { pool, delta };
lrg_object_pool_foreach_active (pool, update_bullet, &update);
#+end_src
...
{
    guint active = lrg_object_pool_get_active_count (pool);
    guint capacity = lrg_object_pool_get_total_size (pool);
    g_print ("Pool: %u/%u (%.1f%%)\n", active, capacity,
