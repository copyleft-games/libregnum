...
:PROPERTIES:
:CUSTOM_ID: singleton-pattern-lrgmcpserver
:END:
...
:PROPERTIES:
:CUSTOM_ID: interface-pattern-toolresource-providers
:END:
...
:PROPERTIES:
:CUSTOM_ID: abstract-base-class-pattern-toolresource-groups
:END:
...
:PROPERTIES:
:CUSTOM_ID: provider-aggregation
:END:
...
:PROPERTIES:
:CUSTOM_ID: tool-registration-flow
:END:
#+begin_example
...
:PROPERTIES:
:CUSTOM_ID: resource-uri-handling
:END:
...
:PROPERTIES:
:CUSTOM_ID: input-injection-architecture
:END:
...
:PROPERTIES:
:CUSTOM_ID: screenshot-capture-flow
:END:
#+begin_example
...
:PROPERTIES:
:CUSTOM_ID: extensibility-points
:END:
*** Adding New Tools
:PROPERTIES:
:CUSTOM_ID: adding-new-tools
:END:
...
:PROPERTIES:
:CUSTOM_ID: adding-new-resources
:END:
...
Register inspectable worlds explicitly with =lrg_engine_register_world()= and
release their registration with =lrg_engine_unregister_world()=. World names
are copied, world references are retained, and duplicate names or registering
one world under multiple names returns false. Registrations survive engine
shutdown/startup; their lifetime belongs to the host's world lifecycle. The
engine does not start updating/drawing worlds merely because they are registered.

MCP ECS operations, engine world registration and game-state mutation belong on
the same engine thread. Save tools use =lrg_save_manager_get_default()= and its
configured directory and saveable registrations. =lrg_save_manager_save_with_description()=
adds display-name metadata while preserving the existing =save()= API.
