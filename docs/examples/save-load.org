...
:PROPERTIES:
:CUSTOM_ID: implementing-an-inventory-saveable
:END:
#+begin_src C
typedef struct {
    gchar *item_id;
...
:PROPERTIES:
:CUSTOM_ID: using-the-save-manager
:END:
#+begin_src C
...
:CUSTOM_ID: see-also
:END:
- [[file:dialog-system.org][Dialog System Example]] - For dialogue state saving
- [[file:quest-tracking.org][Quest Tracking Example]] - For quest progress saving
