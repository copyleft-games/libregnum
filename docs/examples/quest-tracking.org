...
:PROPERTIES:
:CUSTOM_ID: multi-stage-quest-example
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: tracking-quest-progress-in-game-loop
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: displaying-quest-information-in-hud
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: quest-giver-npc-interaction
:END:
#+begin_src C
void interact_with_npc(Game *game, NPC *npc) {
    ...
    for (guint i = 0; i < available_quests->len; i++) {
        ...
        /* Check prerequisites against the journal (the default
         * implementation understands an LrgQuestLog; pass NULL or a
         * GObject of your own when you override the virtual method). */
        if (!lrg_quest_def_check_prerequisites(quest_def, journal)) {
...
:CUSTOM_ID: see-also
:END:
- [[file:dialog-system.org][Dialog System Example]] - For quest-related dialogue
- [[file:save-load.org][Save System Example]] - For persisting quest progress
